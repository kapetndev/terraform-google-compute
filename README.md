...
| `private_service_connect_ranges` | A map of IP address blocks that are allowed to use Private Service Connect | `map(string)` | `{}` | no |
...
| `subnets[*].log_config` | Denotes the logging options for the subnetwork flow logs. If logging is enabled logs will be exported to Stackdriver. This field cannot be set if the purpose of this subnetwork is `INTERNAL_HTTPS_LOAD_BALANCER` | `object{...}` | `{}` | no |
...
| `subnets[*].secondary_ip_range` | An array of configurations for secondary IP ranges for virtual machine instances contained in this subnetwork | `list(object{...})` | `[]` | no |
| `subnets[*].secondary_ip_range[*].ip_cidr_range` | The range of IP addresses belonging to this subnetwork secondary range. Provide this property when you create the subnetwork. Ranges must be unique and non-overlapping with all primary and secondary IP ranges within a network. Only IPv4 is supported | `string` | | yes |
| `subnets[*].secondary_ip_range[*].range_name` | The name associated with this subnetwork secondary range, used when adding an alias IP range to a VM instance. The name must be 1-63 characters long, and comply with RFC1035. The name must be unique within the subnetwork | `string` | | yes |
