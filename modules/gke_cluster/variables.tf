...
variable "kubernetes_version_release_channel" {
  ...
  default     = null
  validation {
    condition     = contains(["RAPID", "REGULAR", "STABLE", "UNSPECIFIED"], var.kubernetes_version_release_channel)
...
EOF
  ...
  default = null
  validation {
    condition     = var.maintenance_policy != null && length(var.maintenance_policy.exclusions) > 3
